...
async function display_player() {
    ...
    for (let i = 0; i < playersData.length; i++) {
        ...
        deleteButton.addEventListener('click', function() {
            deletePlayer(playersData[i].number);
        });
...
async function deletePlayer(playerNumber) {
    let response = await fetch(url);
    let data = await response.json();
    let team = data.find(team => team.id == localStorage.getItem('current_id'));

    // Remove the player from the players array
    team.players = team.players.filter(player => player.number !== playerNumber);

    // Send a PUT request to the server with the updated team data
    let putResponse = await fetch(`${url}/${localStorage.getItem('current_id')}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
